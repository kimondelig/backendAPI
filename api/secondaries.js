...
async function makeRequest(data, res) {
  try {
    ...
    if (response.status === 200) {
        ...
        if (typeof responseData !== 'undefined') {
            ...
            if (salefromdate <= currentDate) {
                // responseData["EVENTDTFAC"].replace("T", " ").slice(0, 19) // moment().format('YYYY-MM-DD HH:mm')
                let eventName = responseData["SSEVENTNAME"] || responseData["ITEMNAME"];
                let data = {
                    "values": [front_end_url, responseData["SOLD_OUT"], venue_name_id, eventcode_id, link_id, data_acc_id, moment().format('YYYY-MM-DD HH:mm'), responseData["EVENTDTFAC"].replace("T", " ").slice(0, 19)],
                    "status": 200 
                }
    ...
    if (error.response) {
        ...
        res.write(`${JSON.stringify({ error: 'No response received from server', message: 'The server did not respond to the request', 'status': 0 })}\n\n`);
    } else {
        // Something happened in setting up the request that triggered an Error
        res.write(`${JSON.stringify({ error: 'Request Setup Error', message: error.message, 'status': 0  })}\n\n`);
    }
    res.end();
    }
}
